...
import {Validator, ACCOUNT_CONSTANT} from 'incognito-chain-web-js/build/wallet';
...
import {decimalDigitsSelector} from '../setting/selectors';
import {
...
import {burnerAddressSelector} from '../account/selectors';
import {navigateToShieldRefund} from '@src/router/NavigationServices';
...
export const mappingTxPortalSelector = createSelector(
  selectedPrivacy,
  decimalDigitsSelector,
  (privacy: any, decimalDigits) => (txp: any) => {
    const {pDecimals = 0, symbol = ''} = privacy || {};
    const {
      amount,
      fee,
      time,
    ...
    let result = {
      ...
      outchainTx: externalTxID
        ? `${CONSTANT_CONFIGS.BTC_EXPLORER_URL}/transaction/${externalTxID}`
        : '',
    };
    ...
    try {
      switch (txType) {
        ...
        case ACCOUNT_CONSTANT.TX_TYPE.SHIELD: {
          ...
          const isShieldProcessing = checkShieldProcessing(
            status,
            decentralized,
          );
          ...
          let data = [
            ...
            },
            {
              ...
              handleOpenUrl: () =>
                LinkingService.openUrlInSide(
                  outChainTx + '?from=incognitochain',
                ),
            },
            {
              label: 'Contract',
        ...
        case ACCOUNT_CONSTANT.TX_TYPE.UNSHIELD: {
          ...
          return [
            ...
            {
              label: 'Outchain fee',
              value: `${outchainFeeStr} ${
                isUnShieldByPToken ? selectedPrivacy.symbol : PRV.symbol
              }`,
