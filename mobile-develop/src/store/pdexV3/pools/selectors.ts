...
import { getPrivacyDataByTokenID as getPrivacyDataByTokenIDSelector } from '@src/store/selectedPrivacy/selectors';
  ...
  (listPools, getPrivacyDataByTokenID, followIds) => {
    ...
    try {
      ...
      pools = listPools.map((pool) => {
        ...
        const originalPrice = convertUtils.toOriginalAmount(
          price,
          token2?.pDecimals,
          true,
        );
