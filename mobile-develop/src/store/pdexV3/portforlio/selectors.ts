...
import {getExchangeRate, getPrincipal, getShareStr} from '../utils';
...
const mapRewardToUSD = ({rewards, getPrivacyDataByTokenID}) => {
  ...
  mapRewards = mapRewards.map(item => {
    ...
    const rewardUSD = toHumanAmount(
      new BigNumber(item.reward).multipliedBy(token.priceUsd).toNumber(),
      token.pDecimals,
    );
    const rewardUSDStr = format.toFixed(rewardUSD, token.pDecimals);
    const rewardUSDSymbolStr = `${rewardUSDStr} ${token.symbol}`;
    const rewardStr = `${format.amountFull(item.reward, token.pDecimals)} ${
      token.symbol
    }`;
    return {
      ...item,
  ...
  (
    ...
    return data.map(item => {
      ...
      let {
        amp,
        apy,
        token1Value: token1PoolValue,
        token2Value: token2PoolValue,
      } = poolDetail || {};
      apy = apy || 0;
      ...
      const principalUSDHuman = new BigNumber(principal.token1USDHuman)
        .plus(principal.token2USDHuman)
        .toNumber();
      const principalUSD = format.amountVer2(
        Math.ceil(
          new BigNumber(principalUSDHuman)
      ...
      const disableBtn =
        isFetchingNFT || !validNFT || !isEnoughNetworkFeeDefault;
      const mapLPRewards =
        mapRewardToUSD({
          rewards: rewards || {},
          getPrivacyDataByTokenID,
        }) || [];
      const mapOrderRewards =
        mapRewardToUSD({
          rewards: orderRewards || {},
          getPrivacyDataByTokenID,
        }) || [];
      const totalRewardUSD = mapLPRewards
        .concat(mapOrderRewards)
        .reduce(
          (prev, curr) => new BigNumber(prev).plus(curr.rewardUSD).toNumber(),
          0,
        );
      const totalRewardAmount = Math.ceil(
        new BigNumber(totalRewardUSD).multipliedBy(Math.pow(10, 9)).toNumber(),
      );
...
export const getDataByShareIdSelector = createSelector(
  listShareSelector,
  listShare => shareId => listShare.find(item => item?.shareId === shareId),
);

export const getDataShareByPoolIdSelector = createSelector(
  listShareSelector,
  listShare => poolId => listShare.find(item => item?.poolId === poolId),
);

export const totalRewardCollectedSelector = createSelector(
  listShareSelector,
  listShare => {
    const rewardUSD = listShare
      .reduce((prev, cur) => {
        return prev.plus(cur.totalRewardUSD);
      }, new BigNumber('0'))
      .toNumber();
    const originalAmount = convertUtils.toOriginalAmount(rewardUSD, 9, true);
...
export const totalContributedUSDSelector = createSelector(
  ...
  listShare => {
    ...
    return format.amountVer2(
      Math.ceil(
        new BigNumber(principalUSD).multipliedBy(Math.pow(10, 9)).toNumber(),
      ),
      9,
    );
  },
