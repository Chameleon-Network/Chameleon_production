...
import { formConfigs, HISTORY_ORDERS_STATE, OPEN_ORDERS_STATE } from './constants';
...
import { getExchangeRate, getOriginalPairRate, getPairRate } from '../utils';
import { getDataByPoolIdSelector, listPoolsVerifySelector } from '../pools/selectors';
import { getPrivacyDataByTokenID as getPrivacyDataByTokenIDSelector, getPrivacyPRVInfo } from '@src/store/selectedPrivacy/selectors';
import { isGettingBalance } from '@src/store/shared/selectors';
import { getInputAmount as getInputTokenAmount } from './utils';
...
export const inpuTokenSelector = createSelector(
  ...
  orderLimitSelector,
  (getPrivacyDataByTokenID, orderLimit) => (field: string) => {
    try {
...
export const feetokenDataSelector = createSelector(
  ...
  (state, { data }, feetoken, getPrivacyDataByTokenID) => {
    try {
      ...
      const feeAmount = toNumber(fee, true) || 0;
      const feeAmountText = fee;
      const origininalFeeAmount = convertUtils.toOriginalAmount(
        feeAmount,
  ...
  (
    ...
    case TAB_BUY_LIMIT_ID: {
      ...
      totalAmount = new BigNumber(buyAmount)
        ...
        .toNumber();
      totalOriginalAmount = convertUtils.toOriginalAmount(
        totalAmount,
    ...
    case TAB_SELL_LIMIT_ID: {
      ...
      const originalSellAmount = convertUtils.toOriginalAmount(
        sellAmount,
          sellInputAmount?.pDecimals,
      );
      sellAmount = toHumanAmount(
        originalSellAmount,
          sellInputAmount?.pDecimals,
      );
