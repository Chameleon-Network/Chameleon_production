...
export const maxAmountValidatorForBuyInput = ({
  ...
  try {
    ...
    const availableBuyOriginalAmount = convertUtils.toOriginalAmount(
      availableBuyAmountNumber,
      buyToken?.pDecimals,
    );
    availableBuyAmountNumber = toHumanAmount(
...
export const getInputAmount = (
  ...
  try {
    ...
    amount = toHumanAmount(originalAmount, token.pDecimals);
    ...
    if (new BigNumber(availableOriginalAmount).isGreaterThan(0)) {
      ...
      availableAmountNumber = toNumber(formatAmount, true);
