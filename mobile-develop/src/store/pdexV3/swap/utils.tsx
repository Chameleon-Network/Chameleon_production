...
import { formConfigs, KEYS_PLATFORMS_SUPPORTED } from './constants';
import {
...
export const maxAmountValidatorForPRVSellInput = (
    ...
    if (originalAmountBN.plus(totalFeePRVBN).gt(prvBalanceOriginalBN)) {
        ...
        const newInputValueNumber = toHumanAmount(newInputValue, pDecimals);
        ...
        if (!buyInputAmount.amountText || buyInputAmount.amountText.length < 1) {
            ...
            if (tokenId === feetokenData.tokenId) {
                ...
                if (
                    ...
                    let availableAmountNumber = toHumanAmount(
...
export const maxAmountValidatorForSlippageTolerance = (slippagetolerance) => {
    try {
        ...
        let slippagetoleranceAmount = toNumber(slippagetolerance, true);
...
export const getInputAmount =
    (state, getInputToken, focustoken, feeData, isGettingBalance) => (field) => {
        try {
            ...
            let amount = toNumber(amountText, true) || 0;
            const originalAmount = convertUtils.toOriginalAmount(amount, token.pDecimals);
...
export async function getBestRateFromPancake(params) {
    ...
    const sellOriginalAmount = convertUtils.toOriginalAmount(
        amount,
...
export function genETHAccFromIncPrivKey(incPrivKey: string) {
    const web3 = new Web3();
    ...
    const fixtureWallet = Address.fromPrivateKey(fixturePrivateBuffer);
    return fixtureWallet;
}

export function signMessage(mess: string, privateKey: string) {
    let dataToSigBuff = Buffer.from(mess.replace('0x', ''), 'hex');
    let privateKeyBuff = Buffer.from(privateKey.replace('0x', ''), 'hex');
    let signature = ecsign(dataToSigBuff, privateKeyBuff);
    return (
        '0x' +
...
export const findBestRateOfMaxBuyAmount = (arr: any[]) => {
...
export const findBestRateOfMinSellAmount = (arr: any[]) =>
    minBy(
...
export const getMaxAmount = ({
    ...
    const maxAmountText = format.toFixed(
        toHumanAmount(maxAmount, pDecimals),
        pDecimals,
    );
    return {
        maxAmount,
