...
import { getAllPrivacyDataSelector, getPrivacyDataByTokenID as getPrivacyDataByTokenIDSelector, getPrivacyPRVInfo, validatePRVBalanceSelector } from '@src/store/selectedPrivacy/selectors';
...
import { defaultAccount, getCurrentPaymentAddressSelector, switchAccountSelector } from '@src/store/account/selectors';
import { checkConvertSelector } from '@src/store/convertToUnifiedToken/selectors';
import { getExchangeRate2 } from '../utils';
...
export const slippagetoleranceSelector = createSelector(
  ...
  (state) => {
    ...
    slippagetolerance = toNumber(slippagetolerance || 0, true);
...
export const feetokenDataSelector = createSelector(
  ...
  (
    ...
    try {
      ...
      let feeAmount = toNumber(fee, true) || 0;
      const feeToNumber = toNumber(fee, true);
      const feeToOriginal = convertUtils.toOriginalAmount(
        feeToNumber,
      ...
      const origininalFeeAmount =
        convertUtils.toOriginalAmount(feeAmount, feeTokenData?.pDecimals, true) || 0;
      const minFeeAmount = toHumanAmount(
        minFeeOriginal,
        feeTokenData?.pDecimals,
      );
      ...
      const minFeeTokenAmount = toHumanAmount(
        minFeeOriginalToken,
      ...
      const availableHunmanAmountToken = toHumanAmount(
        sellAmountToken,
        sellTokenData.pDecimals,
      );
      ...
      const minFeePRVAmount = toHumanAmount(
        minFeeOriginalPRV,
        PRV.pDecimals,
      );
      const minFeePRVFixed = format.toFixed(minFeePRVAmount, PRV.pDecimals);
      const availableHunmanAmountPRV = toHumanAmount(
        sellAmountPRV,
        sellTokenData.pDecimals,
      );
      ...
      const impactOriginalAmount = convertUtils.toOriginalAmount(impactAmount, 2);
  ...
  (
    ...
    try {
      ...
      const networkfeeAmount = format.toFixed(
        toHumanAmount(networkfee, PRV.pDecimals),
        PRV.pDecimals,
      );
...
export const validatePRVNetworkFee = createSelector(
  ...
  (
    ...
    const prvBalance = toNumber(account.value) || 0;
