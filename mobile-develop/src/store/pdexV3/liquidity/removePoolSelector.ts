...
import { getDataByShareIdSelector } from '../portforlio/selectors';
import { isGettingBalance } from '@src/store/shared/selectors';
import { getInputShareAmount } from './utils';
import { getValidRealAmountNFTSelector } from '@src/store/account/selectors';
...
export const maxShareAmountSelector = createSelector(
  ...
  (poolId, shareData, { inputToken, outputToken }) => {
    ...
    const maxInputHuman = toHumanAmount(
      maxInputShare,
    ...
    const maxOutputHuman = toHumanAmount(
      maxOutputShare,
