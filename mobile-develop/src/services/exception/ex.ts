...
import { ErrorCode } from './customError/code';
...
class ExHandler {
  ...
  _getUnexpectedMessageError(exception: any, defaultMessage: any) {
  ...
  showErrorToast(showCode = false) {
    ...
    if (showCode) {
      return ToastService.showError(this.getMessage(msg));
    }
    ...
    try {
      if (this.exception.stackTrace) {
        ...
        if (
          ...
          return `${MESSAGES.PENDING_TX} (${ErrorCode.PENDING_TX})`;
        }

        if (stackCode.indexOf(CODES.INVALID_FEE) === 0) {
          return `${MESSAGES.CAN_NOT_SEND_TX} (${ErrorCode.INVALID_FEE})`;
        }
        ...
        if (
          ...
          return `${MESSAGES.CAN_NOT_SEND_TX} (${ErrorCode.INVALID_ACCOUNT})`;
        }

        if (stackCode.indexOf(CODES.INVALID_TIME) === 0) {
          return `${MESSAGES.CAN_NOT_SEND_TX} (${ErrorCode.INVALID_TIME})`;
        }
      ...
      if (this.exception.code === CODES.NOT_ENOUGH_COIN) {
        return `${MESSAGES.PENDING_TX} (${ErrorCode.PENDING_TX}) (${CODES.NOT_ENOUGH_COIN})`;
      }
  ...
  toastMessageError() {
    return ToastService.showError(this.getMessageError());
  }
