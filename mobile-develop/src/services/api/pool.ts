...
export async function getPoolConfigNoCache() {
  return http
    .get(`pool/staker/configs?${CANCEL_KEY}`)
...
export async function getUserPoolData(paymentAddress, coins) {
  return cachePromise(
    KEYS.PoolUserData(paymentAddress),
    getUserPoolDataNoCache(paymentAddress, coins),
    5000,
  );
}
...
export async function migratePRVProvide({
  paymentAddress,
  signEncode,
  verifyCode,
  amount,
...
export async function checkPreviousProvision({tokenId, paymentAddress}) {
...
export async function withdrawProvision(
  paymentAddress,
  signEncode,
  amount,
  tokenId,
